...
func mainMenu() {
	...
	// can break line aftr comma, opening parenthesis,
...
func deleteCategory() {
	...
	var userCatogory string
	// var indexToRm []int6

	fmt.Println("Please enter the category you want to delete: ")
	fmt.Scan(&userCatogory)

	// change to Title format for strings input
	userCatogory = strings.Title(strings.ToLower(userCatogory))
	if index, found := checkCategory(userCatogory, category); !found {
		fmt.Println(userCatogory + " category does not exists")
	} else {

		removeItemFromMap(userCatogory, index)
		modifyItemContents(index)
		fmt.Println("Deleting", userCatogory, "from the shopping list")
		// delayThreeSeconds()
	}
}

func removeItemFromMap(cat string, index int) {
